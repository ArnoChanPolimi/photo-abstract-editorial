...
**将一张照片转化为「原始摄影区域 + 抽象记忆面板」的竖向编辑作品，默认不添加标题**
...
这是一个 **Codex Skill**，它能将一张照片转化为「原始摄影区域 + 抽象记忆面板」的竖向编辑作品，或仅输出独立抽象面板。作品默认不含文字；只有用户明确要求时才添加标题。
...
4. Skill 会将原图保留在成品的上方或主要区域，并在下方创建由原图关系推导出的极简抽象面板。成品默认不含文字；只有明确要求时才添加一个原创英文标题或副标题。
...
| **📝 版式与文字** | 默认无文字；明确要求标题时，可调整标题位置、字体气质、长度和是否使用副标题 |
...
将一张照片转化为“原始摄影区域 + 抽象记忆面板”的竖向编辑作品的 Codex Skill。作品默认不添加标题。它保留照片的真实内容，并仅从照片本身提炼空间关系、构图节奏和色彩关系；它不是滤镜、照片重画或风格迁移。

将上传的图片严格作为唯一的内容来源和摄影原片使用。根据 [output-formats.md](output-formats.md) 中选择的作品形态，生成“原照片区域＋抽象记忆面板”的完整编辑作品、独立的照片衍生抽象作品，或同时包含两者的配套组合。默认不生成标题或任何其他文字。不得生成与照片无关的抽象画，也不得直接对照片应用滤镜。
...
在完整编辑作品或配套组合模式下，生成一张由上方摄影区域和下方象牙色抽象面板组成的竖向作品。在独立抽象模式下，省略照片，并根据用户选择将同一母题适配到独立的象牙色或透明画布；不得直接从完整作品中裁切母题充当独立输出。
...
默认不要创作或渲染标题。仅当用户明确要求标题时才应用本节：根据照片主体、空间关系、时间、光线、运动和情绪，在内部创作一个原创英文标题。
...
仅当用户明确要求副标题，并且副标题能够增加新的语义层而不是重复主标题时，才允许加入一个 3–7 个英文单词的短副标题。主标题与副标题均只出现一次。

先在内部确定最终标题，再将其准确渲染。不得生成候选标题、解释文字或其他文案。

## 九、可选标题排版

仅当用户明确要求标题时才应用本节。标题只能放在象牙色抽象面板内，位于抽象母题下方或侧边，具体位置应当参考两侧留白和整体协调情况。
...
仅生成 [output-formats.md](output-formats.md) 中用户选择的作品形态：完整编辑作品、独立抽象作品或两者配套组合，并仅导出所选格式。独立模式必须省略原照片，同时保留从照片提炼的关系；可按选择输出带可选标题的象牙色画布，或无标题的透明底纯母题。若用户要求 SVG 或矢量 PDF，必须将抽象标记与标题重构为真正的矢量路径或图元；不得仅嵌入、裁切或自动描摹位图后将其称为矢量。完整作品的 SVG 或 PDF 必然包含作为位图嵌入的原照片，因此不能称为全矢量作品。

默认绝对不出现任何文字。若用户明确要求标题或副标题，只允许出现该项指定文字；不得出现数字、日期、编号、地点说明、色卡、图例、签名、logo 或水印。
